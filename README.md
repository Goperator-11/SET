...
상단에서 **방어 / 공격** 트랙을 전환합니다. 진도는 트랙마다 따로 쌓이고,
**계급·포인트·상점·팀·연속 학습일은 한 계정에서 공유**합니다.

| 트랙 | 내용 | 분량 |
|---|---|---|
| **방어 (BLUE)** | SOC·블루팀·DFIR — 로그 분석부터 위협 헌팅까지 | 160일 (완성) |
| **공격 (RED)** | 레드팀·침투·CTF — 정찰·웹·권한상승·pwn·리버싱·암호·네트워크·클라우드 | ACT 0~8, **140일** (확장 중, 목표 ~200일) |

공격 코스는 **CTF·대회용 문제 형식**입니다. 정찰(nmap·gobuster·enum4linux),
웹 공격(Burp·sqlmap·ffuf), 익스플로잇(Metasploit·searchsploit),
패스워드(hydra·hashcat) 등 실전 도구 사용을 문제로 익힙니다.
모든 실습은 **허가된 대상(본인 실습실·CTF·버그바운티 scope)** 전제입니다.
