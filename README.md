...
140일 · **508문항** · 989개 실습 과제. 20 / 40 / 60 / 80 / 100 / 120 / 140일차는 **보스 문제**이고,
...
- **XP와 계급 15단계** — 신입 관제원 → 로그 판독병 → … → SOC 아키텍트 → 야간근무의 전설
- **워게임 7종** — 힌트 없이 실전 로그를 분석해 답을 제출하는 CTF 모드 (1,530점)
- **도구 레퍼런스 22종** — 설치 명령, 첫 실행 5분, 명령어 170개 치트시트
- **연속 학습일** — 잔디로 표시
- **배지 21종** — 액트 클리어, 연속 학습, 보스 처치, 실전 랩 해결, 첫 제출 정답 누적 등
- **채점 현황** — 제출 기록 전체와 정답률
